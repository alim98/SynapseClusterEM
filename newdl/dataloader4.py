...
class SynapseDataLoader:
    ...
    def create_segmented_cube(
        ...
        vesicle_fill_threshold: float = 95.0,  # Required percentage of vesicle fill for type 12 (default 95%)
    ) -> np.ndarray:
        ...
        if segmentation_type == 0:
            ...
            min_required_pixels = 25 * 25 * 25  # Minimum required for a full box
            if vesicle_pixel_count < min_required_pixels:
                print(f"Discarding sample: Vesicle cloud too small ({vesicle_pixel_count} pixels < {min_required_pixels} minimum required)")
                return None
            
            # Get coordinates of the vesicle cloud
            vesicle_coords = np.where(vesicle_cloud)
            if len(vesicle_coords[0]) > 0:
                # Find the bounds of the vesicle cloud
                z_min_vc, z_max_vc = np.min(vesicle_coords[0]), np.max(vesicle_coords[0])
                y_min_vc, y_max_vc = np.min(vesicle_coords[1]), np.max(vesicle_coords[1])
                x_min_vc, x_max_vc = np.min(vesicle_coords[2]), np.max(vesicle_coords[2])
                
                # Calculate the dimensions of the vesicle cloud
                z_size = z_max_vc - z_min_vc + 1
                y_size = y_max_vc - y_min_vc + 1
                x_size = x_max_vc - x_min_vc + 1
                
                print(f"Vesicle cloud dimensions: {z_size}×{y_size}×{x_size}")
                
                # Check if the vesicle cloud is large enough in each dimension to fit a 25×25×25 box
                if z_size < 25 or y_size < 25 or x_size < 25:
                    print(f"Discarding sample: Vesicle cloud dimensions too small to fit 25×25×25 box")
                    return None
                ...
                # Check if we can fit a full 25×25×25 box
                actual_z_size = z_max - z_min
                actual_y_size = y_max - y_min
                actual_x_size = x_max - x_min
                
                if actual_z_size < 25 or actual_y_size < 25 or actual_x_size < 25:
                    print(f"Discarding sample: Cannot fit a 25×25×25 box within the volume bounds")
                    print(f"Available box dimensions: {actual_z_size}×{actual_y_size}×{actual_x_size}")
                    return None
                
                # Set the box region to True
                box_mask[z_min:z_max, y_min:y_max, x_min:x_max] = True
                
                # Intersection of vesicle cloud and the box - this is our segmentation mask
                combined_mask_full = np.logical_and(vesicle_cloud, box_mask)
                
                # Check if there are enough vesicle pixels in the box
                vesicle_in_box_count = np.sum(combined_mask_full)
                box_size_actual = np.sum(box_mask)
                box_percentage = (vesicle_in_box_count / box_size_actual) * 100.0
                
                print(f"Vesicle pixels in 25×25×25 box: {vesicle_in_box_count} out of {box_size_actual} ({box_percentage:.2f}%)")
                
                # Success - we have a vesicle cloud that fits in a 25×25×25 box
                print(f"Successfully created 25×25×25 bounding box for vesicle cloud")
            else:
                # If no vesicle cloud is found (shouldn't happen due to earlier check)
