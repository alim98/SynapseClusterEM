...
def compare_segmentation_types():
    """
    Visualize and compare segmentation types 10, 11, and 12 in a single image as GIFs
    for each fixed sample.
    """
    # Initialize and parse configuration
    config.parse_args()
    ...
    logger.info("Starting segmentation type comparison for fixed samples...")
    ...
    for sample in fixed_samples:
        ...
        var1 = sample['Var1']
        sample_id = sample['id']
        slice_number = sample.get('slice_number', None)  # Use provided slice number if available
        
        logger.info(f"Processing sample {sample_id}: {var1} from {bbox_name}")
        ...
        if os.path.exists(os.path.join(config.excel_file, f"{bbox_name}.xlsx")):
            syn_df = pd.read_excel(os.path.join(config.excel_file, f"{bbox_name}.xlsx"))
            sample_row = syn_df[syn_df['Var1'] == var1]
            
            if sample_row.empty:
                logger.warning(f"No data found for {var1} in {bbox_name}")
                continue
            
            # Extract coordinates
            try:
                # Check for different column naming conventions
                logger.info(f"Available columns: {list(sample_row.columns)}")
                ...
                central_coord = (
                    int(sample_row[x_col].values[0]), 
                    int(sample_row[y_col].values[0]), 
                    int(sample_row[z_col].values[0])
                )
                side1_coord = (
                    int(sample_row[x1_col].values[0]), 
                    int(sample_row[y1_col].values[0]), 
                    int(sample_row[z1_col].values[0])
                )
                side2_coord = (
                    int(sample_row[x2_col].values[0]), 
                    int(sample_row[y2_col].values[0]), 
                    int(sample_row[z2_col].values[0])
                )
            ...
            logger.warning(f"No Excel file found for {bbox_name}. Using default coordinates.")
            # Fallback to center of volume
            center_z = raw_vol.shape[0] // 2
            center_y = raw_vol.shape[1] // 2
            center_x = raw_vol.shape[2] // 2
            central_coord = (center_x, center_y, center_z)
        ...
        for frame_idx in range(num_frames):
            ...
            titles = {
                10: "Type 10: Cleft + Presynaptic",
                11: "Type 11: Cleft + Presynaptic (No Mito)",
            ...
            for i, seg_type in enumerate(valid_seg_types):
                ...
                ax.set_title(titles[seg_type])
                ax.axis('off')
            
            # Add sample information as a suptitle
            plt.suptitle(f"Sample {sample_id}: {bbox_name} - {var1}", fontsize=14)
            plt.tight_layout()
        ...
        # Save as GIF
        gif_path = os.path.join(output_dir, f"{bbox_name}_{var1}_comparison.gif")
    ...
    logger.info("Segmentation type comparison completed")
