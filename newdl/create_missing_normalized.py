...
logger = logging.getLogger("create_missing_normalized")

def main():
    # Initialize and parse configuration
    config.parse_args()
    
    # Sample and weights to process
    bbox_name = "bbox3"
    var1 = "non_spine_synapse_009"
    weights_to_process = [0.3, 0.7]  # Only missing weights
    ...
    for weight in weights_to_process:
        logger.info(f"Processing {var1} from {bbox_name} with weight {weight} and size normalization...")
        try:
            visualize_comparison(
                ...
                target_percentage=0.15,  # Explicit target percentage
                size_tolerance=0.1
            )
            
            # Check if files were created
            sample_key = f"{bbox_name}_{var1}"
            path = os.path.join(save_gifs_dir, f"{sample_key}_intelligent_w{weight}_normalized.gif")
            ...
            logger.error(f"Error processing with size normalization weight={weight}: {e}")
            import traceback
